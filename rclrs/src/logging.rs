...
mod tests {
    ...
    test_with_executors! {
        fn test_rosout_publishing_default(executor, node_name) -> Result<(), RclrsError> {
            use crate::rcl_bindings::rcl_logging_rosout_enabled;
            use ros_env::rcl_interfaces::msg::rmw::Log;
            use std::sync::{Arc, Mutex};

            let namespace = format!("/{node_name}");
            let node = executor
    ...
    test_with_executors! {
        fn test_rosout_disabled(executor, node_name) -> Result<(), RclrsError> {
            use ros_env::rcl_interfaces::msg::rmw::Log;
            use std::sync::{
