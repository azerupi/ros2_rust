...
pub(crate) type SetOnReadyFn<H> =
    unsafe fn(&H, rcl_event_callback_t, *const c_void) -> rcl_ret_t;
...
pub(crate) struct OnReadyRegistration<H: Send + Sync + 'static> {
    ...
    // Field order is load-bearing for teardown safety: `handle` is declared
    ...
    // and only *after* `handle` above (declaration order = drop order) has
    // finalized the rcl entity. The drop side-effect is the point, not the value.
    #[allow(dead_code)]
    ctx: CtxBox,
}
...
impl<H: Send + Sync + 'static> Drop for OnReadyRegistration<H> {
    fn drop(&mut self) {
        ...
        // *after* `handle` (field order), so the rcl entity is finalized before
...
/// `handle` Arc — see the field-order note there.
