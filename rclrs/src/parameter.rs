...
///
/// Captured as a function pointer at declaration time so that the parameter map can enforce
/// the declared Rust type without knowing what it is. See [`ParameterVariant::validate_value`].
fn type_check_of<T: ParameterVariant>(value: &ParameterValue) -> Result<(), String> {
...
struct DeclaredStorage {
    ...
    /// Enforces the Rust type the parameter was declared with. The [`kind`](Self::kind) alone
    /// is not enough: a value can have the right kind and still be unrepresentable in `T`.
    type_check: fn(&ParameterValue) -> Result<(), String>,
...
impl std::fmt::Display for ParameterValueError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ParameterValueError::Invalid(reason) => write!(f, "parameter value is not valid for this parameter's type: {reason}"),
                ...
                match entry.get_mut() {
                    ParameterStorage::Declared(param) => {
                        if T::kind() == param.kind {
                            ...
                            // `T` here is the type of the value being set, which is not
                            // necessarily the type the parameter was declared with. The two only
                            // have to share a kind, so enforce the declared type as well.
...
mod tests {
    ...
    fn test_service_path_rejects_value_of_right_kind_but_wrong_type() {
        ...
        let err = map
            ...
            .unwrap_err();
        assert!(
            err.contains("not valid for this parameter's type") && err.contains("on, off"),
            "unhelpful rejection reason: {err}"
        );
