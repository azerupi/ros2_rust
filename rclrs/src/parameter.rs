...
///
/// Captured as a function pointer at declaration time so that the parameter map can enforce the
/// declared Rust type without knowing what it is. The conversion the type already has to provide
/// *is* the check, so there is nothing for a type to implement beyond [`TryFrom`], and no way for
/// it to accept a value that a later read would then reject.
