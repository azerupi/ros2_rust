...
//! that do not fit are rejected by [`ParameterVariant::validate_value`] before they are ever
//! stored, so a parameter declared as `u16` can never hold something that is not one.
//!
    ...
    ($($t:ty),* $(,)?) => { $(
        ...
        impl From<ParameterRange<$t>> for ParameterRanges {
            fn from(range: ParameterRange<$t>) -> Self {
                ParameterRange::<i64> {
                    lower: range.lower.map(i64::from),
                    upper: range.upper.map(i64::from),
...
impl_vec_of_convertible!(
    ...
    (
        ...
        Double,
        DoubleArray,
        |value: DurationSecs| { value.0.as_secs_f64() }
    ),
    (
        DurationMillis,
        IntegerArray,
        Integer,
        IntegerArray,
        |value: DurationMillis| { whole_millis(value.0) }
    ),
);
...
/// A [`Duration`] parameter expressed in seconds, as a ROS 2 double.
///
/// [`Duration`] itself is not a parameter type, because the unit it would be stored in is not
/// something to leave implicit in a robotics configuration. Choose the unit at the point of
/// declaration by using this or [`DurationMillis`]. Both dereference to [`Duration`], and the
/// unit is reported in the parameter descriptor's constraints.
///
/// Ranges are expressed in seconds, e.g. `range = 0.0..=5.0`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct DurationSecs(pub Duration);

/// A [`Duration`] parameter expressed in whole milliseconds, as a ROS 2 integer.
///
/// See [`DurationSecs`] for why the unit is part of the type. Sub-millisecond precision is lost.
/// Ranges are expressed in milliseconds, e.g. `range = 0..=5000`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct DurationMillis(pub Duration);

macro_rules! impl_duration_wrapper {
    ($($t:ident),* $(,)?) => { $(
        impl Deref for $t {
...
impl_duration_wrapper!(DurationSecs, DurationMillis);

impl From<DurationSecs> for ParameterValue {
    fn from(value: DurationSecs) -> Self {
        ParameterValue::Double(value.0.as_secs_f64())
    }
}

impl TryFrom<ParameterValue> for DurationSecs {
    type Error = ParameterValueError;
        ...
        match value {
            ParameterValue::Double(v) => {
                Duration::try_from_secs_f64(v)
                    ...
                    .map_err(|_| {
                        ParameterValueError::Invalid(format!(
                        "{v} is not a valid duration in seconds; must be finite and not negative"
                    ))
                    })
            }
            _ => Err(ParameterValueError::TypeMismatch),
...
impl ParameterVariant for DurationSecs {
    ...
    fn type_constraints() -> Option<Arc<str>> {
        Some("a duration in seconds".into())
    }
}

/// The whole milliseconds of a duration, as a ROS 2 integer.
fn whole_millis(value: Duration) -> i64 {
    let millis = value.as_millis();
    debug_assert!(
        millis <= i64::MAX as u128,
        "duration of {millis} ms cannot be represented as a ROS 2 integer parameter",
    );
    i64::try_from(millis).unwrap_or(i64::MAX)
}

impl From<DurationMillis> for ParameterValue {
    fn from(value: DurationMillis) -> Self {
        ParameterValue::Integer(whole_millis(value.0))
    }
}

impl TryFrom<ParameterValue> for DurationMillis {
    type Error = ParameterValueError;

    fn try_from(value: ParameterValue) -> Result<Self, Self::Error> {
        match value {
            ParameterValue::Integer(v) => u64::try_from(v)
                .map(|millis| DurationMillis(Duration::from_millis(millis)))
                .map_err(|_| {
                    ParameterValueError::Invalid(format!(
                        "{v} is not a valid duration in milliseconds; must not be negative"
                    ))
                }),
            _ => Err(ParameterValueError::TypeMismatch),
        }
...
impl ParameterVariant for DurationMillis {
    ...
    fn type_constraints() -> Option<Arc<str>> {
        Some("a duration in milliseconds".into())
    }
}
...
mod tests {
    ...
    fn test_round_trips() {
        ...
        round_trip(vec![DurationSecs(Duration::from_millis(250))]);
        round_trip(vec![DurationMillis(Duration::from_millis(30))]);
        ...
        round_trip(DurationSecs(Duration::from_millis(1500)));
        round_trip(DurationMillis(Duration::from_millis(1500)));
    }
    ...
    #[test]
    fn test_durations_reject_invalid_values() {
        assert!(DurationSecs::try_from(ParameterValue::Double(-1.0)).is_err());
        assert!(DurationSecs::try_from(ParameterValue::Double(f64::NAN)).is_err());
        assert!(DurationMillis::try_from(ParameterValue::Integer(-1)).is_err());
    }
    ...
    fn test_arrays_check_every_element() {
        ...
        assert!(
            Vec::<DurationMillis>::try_from(ParameterValue::IntegerArray(vec![-1].into())).is_err()
        );
    ...
    fn test_arrays_report_the_constraints_of_their_elements() {
        ...
        assert_eq!(
            Vec::<DurationMillis>::type_constraints().as_deref(),
            Some("every element a duration in milliseconds")
        );
    ...
    fn test_declaring_parameters_with_ergonomic_types() {
        ...
        let timeout: MandatoryParameter<DurationSecs> = node
            .declare_parameter("timeout")
            .default(DurationSecs(Duration::from_millis(2500)))
            .mandatory()
            .unwrap();
        assert_eq!(timeout.get().as_millis(), 2500);
