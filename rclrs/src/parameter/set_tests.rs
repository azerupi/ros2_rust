...
struct DefaultedConfig {
    ...
    /// A value supplied for the set as a whole wins over this.
    #[param(default = 7.0)]
    explicit: f64,
...
fn test_defaults_can_come_from_a_default_impl() {
    ...
    // The supplied value is authoritative: it is specific to this instance of the set, whereas
    // the field attribute says what the type defaults to in general.
    assert_eq!(config.explicit, 999.0);
