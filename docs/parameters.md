...
`validate` also applies to a `read_only` parameter, which is declared once and never set again but
still takes its one value from a parameter file or the command line. A value the callback rejects
makes the declaration fail, with the reason the callback gave. `on_change` is the one that has no
meaning on a read-only parameter, since there is never a change to react to.
...
Three places can supply a default for the same field, and they apply in this order:

1. A value supplied for one instance of the set, by a parent field's `default`.
2. The field's own `#[param(default = ...)]`.
3. The set's `#[parameters(default = ...)]`.

A value supplied by a parent is specific to that one use of the set, so it wins over anything the
set says about itself, which is what lets a parent configure a set it did not write. The other two
both describe the type in general, so the more specific of them wins, which is the field's:
